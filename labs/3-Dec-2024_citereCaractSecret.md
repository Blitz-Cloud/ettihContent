...
int main(){
	int sisteme[3]={0},nrDeSysAccesate=0,i,j;
	char passwords[3][5]={"1234\0","1423\0","1919\0"};
	char passwd[5];
	printf("Pinul pentru primul sistem\n");
	for(i=0; i<3 && strcmp(passwd,passwords[0]); i++) {
		for(j=0; j<4; j++) {
			passwd[j]=getch();
		}
		passwd[4]='\0';
		if(i==2 && strcmp(passwd,passwords[0])) {
			printf("Accesul la sistem este blocat\n");
		}
		else if(!strcmp(passwd,passwords[0])) {
			sisteme[0]=1;;
			nrDeSysAccesate++;
		}else if(strcmp(passwd,passwords[0])) {
			printf("Introdu din nou parola\n");
		}
	}
	printf("Pinul al doilea sistem\n");
	for(i=0; i<3 && strcmp(passwd,passwords[1]); i++) {
		for(j=0; j<4; j++) {
			passwd[j]=getche();
		}
		passwd[4]='\0';
		printf("\n");
		if(i==2 && strcmp(passwd,passwords[1])) {
			printf("Accesul la sistem este blocat\n");
		}
		else if(!strcmp(passwd,passwords[1])) {
			sisteme[1]=1;
			nrDeSysAccesate++;
		}else if(strcmp(passwd,passwords[1])) {
			printf("Introdu din nou parola\n");
	...
	printf("Pinul celui de al treilea sistem\n");
	for(i=0; i<3 && strcmp(passwd,passwords[2]); i++) {
		for(j=0; j<4; j++) {
			passwd[j]=getch();putchar('*');
		}
		printf("\n");
		passwd[4]='\0';
		if(i==2 && strcmp(passwd,passwords[2])) {
			printf("Accesul la sistem este blocat\n");
		}
		else if(!strcmp(passwd,passwords[2])) {
			sisteme[2]=1;
			nrDeSysAccesate++;
		}else if(strcmp(passwd,passwords[2])) {
			printf("Introdu din nou parola\n");
